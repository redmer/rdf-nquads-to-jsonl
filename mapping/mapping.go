...
func (m *Mapper) Add(q parser.Quad) {
	...
	newType, hasStringLike, lang, stringLen := inferType(q.Object)
...
func inferType(obj interface{}) (fieldType FieldType, hasStringLike bool, lang string, stringLen int) {
	switch obj := obj.(type) {
	case bool:
		return TypeBool, false, "", 0
	case int, int64:
		return TypeLong, false, "", 0
	case float32, float64:
		return TypeDouble, false, "", 0
	case parser.Date, parser.DateTime:
		return TypeDate, false, "", 0
	case parser.URI, parser.AnyURI:
		return TypeKeyword, false, "", 0
	case parser.LangString:
		return TypeUnknown, true, obj.BaseLang(), len(obj.Value)
	case string:
		return TypeUnknown, true, "", len(obj)
	default:
		return TypeUnknown, false, "", 0
	}
}
